/*
1. board의 빈 공간과 table의 블록을 모두 추출한다. BFS를 이용하며, 좌표를 추출해서 list에 넣어준다.
2. 추출한 table의 블록들을 board의 빈공간과 하나씩 비교해주면서 딱 맞아들어가면 visited 배열을 true로 바꿔주고(중복 방지) 블록 개수만큼 answer에 더해준다. table 블록 좌표 개수와 board 빈공간 좌표 개수가 맞지않거나(블록개수가 다름을 의미, 이미 채워진 공간이면(visited 배열 체크) 확인하지 않는다.

(*)
1. 블록과 빈공간의 좌표를 비교해가며 모양이 일치하는지 확인하기
위해, (0, 0)을 기준점으로 하며, 블록을 회전시키면 기준점이 달라
지기 때문에, 이에 따른 보정 과정이 필요
2. 블록과 빈공간의 좌표를 모두 오름차순 정렬
*/
